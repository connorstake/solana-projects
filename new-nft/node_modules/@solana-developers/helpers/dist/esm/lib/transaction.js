...
export const getSimulationComputeUnits = async (connection, instructions, payer, lookupTables, commitment = "confirmed") => {
 ...
 * ```
 */
async function sendTransactionWithRetry(connection, transaction, signers, { maxRetries = DEFAULT_SEND_OPTIONS.maxRetries, initialDelayMs = DEFAULT_SEND_OPTIONS.initialDelayMs, commitment = DEFAULT_SEND_OPTIONS.commitment, skipPreflight = DEFAULT_SEND_OPTIONS.skipPreflight, onStatusUpdate = (status) => console.log("Transaction status:", status), } = {}) {
 ...
 * ```
 */
export async function prepareTransactionWithCompute(connection, tx, payer, priorityFee = 10000, computeUnitBuffer = {}, commitment = "confirmed") {
    tx.add(ComputeBudgetProgram.setComputeUnitPrice({
        microLamports: priorityFee,
    }));
    const simulatedCompute = await getSimulationComputeUnits(connection, tx.instructions, payer, [], commitment);
 ...
 */
export async function getIdlParsedAccountData(idlPath, accountName, accountAddress, connection) {
 ...
 */
export async function parseAnchorTransactionEvents(idlPath, signature, connection) {
    const fs = await import("node:fs");
    const path = await import("node:path");
    const idlFile = fs.readFileSync(path.resolve(idlPath), "utf8");
    const idl = JSON.parse(idlFile);
 ...
 */
export async function decodeAnchorTransaction(idlPath, signature, connection) {
    const fs = await import("node:fs");
    const path = await import("node:path");
    const idlFile = fs.readFileSync(path.resolve(idlPath), "utf8");
    const idl = JSON.parse(idlFile);
    ...
    const instructions = "version" in message
                    ...
                    const accounts = await Promise.all(accountIndices.map(async (index, i) => {
                        ...
                        if (accountInfo?.owner.equals(program.programId)) {
                            try {
                                ...
                                if (accountType) {
                                    accountData = accountsCoder.decode(accountType.name, accountInfo.data);
                                }
                            }
                            catch (e) {
 ...
 * ```
 */
export async function sendTransaction(connection, transaction, signers, priorityFee = 10000, options = {}) {
    ...
    const hasComputeInstructions = transaction.instructions.some((ix) => ix.programId.equals(ComputeBudgetProgram.programId));
    ...
    await prepareTransactionWithCompute(connection, transaction, transaction.feePayer, priorityFee, computeUnitBuffer, commitment);
    return sendTransactionWithRetry(connection, transaction, signers, {
        commitment,
        ...sendOptions,
    });
