...
exports.prepareTransactionWithCompute = prepareTransactionWithCompute;
 ...
 * ```
 */
async function prepareTransactionWithCompute(connection, tx, payer, priorityFee = 10000, computeUnitBuffer = {}, commitment = "confirmed") {
    tx.add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({
        microLamports: priorityFee,
    }));
    const simulatedCompute = await (0, exports.getSimulationComputeUnits)(connection, tx.instructions, payer, [], commitment);
 ...
 */
async function getIdlParsedAccountData(idlPath, accountName, accountAddress, connection) {
    ...
    const provider = connection
        ? new anchor_1.AnchorProvider(connection, anchor_1.AnchorProvider.env().wallet, {})
 ...
 */
async function parseAnchorTransactionEvents(idlPath, signature, connection) {
    const fs = await Promise.resolve().then(() => __importStar(require("node:fs")));
    const path = await Promise.resolve().then(() => __importStar(require("node:path")));
    const idlFile = fs.readFileSync(path.resolve(idlPath), "utf8");
    const idl = JSON.parse(idlFile);
 ...
 */
async function decodeAnchorTransaction(idlPath, signature, connection) {
    const fs = await Promise.resolve().then(() => __importStar(require("node:fs")));
    const path = await Promise.resolve().then(() => __importStar(require("node:path")));
    const idlFile = fs.readFileSync(path.resolve(idlPath), "utf8");
    const idl = JSON.parse(idlFile);
    const provider = connection
        ...
        : anchor_1.AnchorProvider.env();
    const program = new anchor_1.Program(idl, provider);
    const accountsCoder = new anchor_1.BorshAccountsCoder(idl);
...
async function sendTransaction(connection, transaction, signers, priorityFee = 10000, options = {}) {
    ...
    const hasComputeInstructions = transaction.instructions.some((ix) => ix.programId.equals(web3_js_1.ComputeBudgetProgram.programId));
